...
describe('Schema validator', () => {
  ...
  describe('enum validation', () => {
    it('accepts value in enum list', () => {
      const result = validate(
        { level: 'info' },
        { level: { type: 'string', enum: ['debug', 'info', 'warn', 'error'] } }
      );
      expect(result.ok).toBe(true);
      expect(result.value).toEqual({ level: 'info' });
    });
    ...
    it('rejects value not in enum list', () => {
      ...
      const errors = result.error.context.errors;
      expect(errors).toHaveLength(1);
      expect(errors[0].code).toBe('ENUM_INVALID');
      expect(errors[0].key).toBe('level');
    });
    ...
    it('type check runs before enum check', () => {
      const result = validate(
        { level: 'info' },
      ...
      expect(result.ok).toBe(false);
      const errors = result.error.context.errors;
      expect(errors[0].code).toBe('TYPE_MISMATCH'); // NOT ENUM_INVALID
    });

    it('enum works with nested objects', () => {
      const result = validate(
        { db: { mode: 'invalid' } },
        {
          db: {
      ...
      expect(result.ok).toBe(false);
      const errors = result.error.context.errors;
      expect(errors.some(e => e.code === 'ENUM_INVALID')).toBe(true);
      expect(errors.some(e => e.key === 'db.mode')).toBe(true);
    });
  });
});
