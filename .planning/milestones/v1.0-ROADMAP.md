...
**Goal**: Transform Dynamo from bootstrap-per-invocation to a persistent daemon runtime, add Linotype prompt template library, rewrite CLI/hooks as thin clients. This corrects the root cause of all Phase 16 E2E failures and establishes prompt engineering as first-class infrastructure.
**Depends on**: Phase 16
**Requirements**: INF-01, SVC-01, SVC-08, MOD-01
**PRD**: .claude/unfuck-the-whole-damn-thing.md
**Success Criteria** (what must be TRUE):
  1. `bun bin/dynamo.cjs start` launches a persistent daemon process that survives the parent shell
  2. `bun bin/dynamo.cjs hook SessionStart` forwards to daemon via HTTP and returns response (not full bootstrap)
  3. Hooks return `{}` with exit 0 when daemon is not running (no Claude Code errors)
  4. Hooks return error with exit 1 when daemon is running but handler fails (noise when broken)
  5. Linotype parses markdown templates with JSON frontmatter and resolves `{{slot}}`, `{{#if}}`, `{{#each}}`, `{{> partial}}` syntax
  6. All 15+ Reverie prompt string literals extracted to `modules/reverie/prompts/*.md` templates
  7. `bun bin/dynamo.cjs reverie enable` spawns Secondary + Tertiary terminal windows as a linked triad
  8. DuckDB writes funnel through single daemon Ledger instance — no contention
  9. `bun bin/dynamo.cjs stop` gracefully shuts down daemon with REM completion
**Plans**: 10 plans

Plans:
- [ ] 17-01-PLAN.md -- Linotype types, parser, and template engine (Wave 1a core)
- [ ] 17-02-PLAN.md -- Linotype composer, validator, and public API barrel (Wave 1a completion)
- [ ] 17-03-PLAN.md -- Daemon lifecycle management and entry point (Wave 1b part 1)
- [ ] 17-04-PLAN.md -- Daemon HTTP server with Wire relay merge (Wave 1b part 2)
- [ ] 17-05-PLAN.md -- Thin client rewrite and Exciter dispatchHook (Wave 2)
- [ ] 17-06-PLAN.md -- Framework template contracts, SDK template registry, module lifecycle (Wave 3)
- [ ] 17-07-PLAN.md -- Prompt extraction: formation, recall, backfill templates (Wave 4a part 1)
- [ ] 17-08-PLAN.md -- Prompt extraction: framing, REM, sublimation, face prompt (Wave 4a part 2)
- [ ] 17-09-PLAN.md -- Skills, hook handler dispatch, module register, config (Wave 4b)
- [ ] 17-10-PLAN.md -- E2E validation and regression testing (Wave 5)

## Progress

**Execution Order:**
Phases execute in numeric order: 1 -> 2 -> 3 -> 3.1 -> 3.2 -> 4 -> 5 -> 6 -> 7-16 (M2) -> 17
...
| 7-16. M2 Reverie Module | 54/55 | Phase 16 paused | - |
| 17. Persistent Runtime & Prompt Infrastructure | 0/10 | Not started | - |
